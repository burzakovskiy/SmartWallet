...
/**
 * Holds methods for expense categories card management
 *
 * @author Roman Burzakovskiy
 */
public class CategoryExpenseCard extends Card {
    private OnCategoryRemoveListener onCategoryRemoveListener;
    private HashMap<String, List<String>> mExpenseMap;

    public CategoryExpenseCard(Context context, HashMap<String, List<String>> expenseMap, OnCategoryRemoveListener onCategoryRemoveListener) {
        super(context, R.layout.category_management_expense_card_main_inner_layout);
        mExpenseMap = expenseMap;
        this.onCategoryRemoveListener = onCategoryRemoveListener;
        CategoryExpenseCardHeader header = new CategoryExpenseCardHeader(getContext());
    ...
    public void setupInnerViewElements(ViewGroup parent, View view) {
        ...
        for (int i = FinanceDocument.NUMBER_OF_CATEGORIES - FinanceDocument.NUMBER_OF_EXPENSE_CATEGORIES; i <= FinanceDocument.NUMBER_OF_CATEGORIES; i++) {
            wrapperLayout.addView(generateCategoryRow(String.valueOf(i)));
        }

        //generating custom categories list
        for (Map.Entry<String, List<String>> entry : mExpenseMap.entrySet()) {
    ...
    private RelativeLayout generateCustomCategoryRow(final String categoryId, String name) {
        ...
        Drawable cardIcon = ContextCompat.getDrawable(getContext(), R.drawable.ic_minus_black_24dp);
    ...
    private class CategoryExpenseCardHeader extends CardHeader {
        public CategoryExpenseCardHeader(Context context) {
            super(context, R.layout.category_management_expense_card_header_inner_layout);
        }
